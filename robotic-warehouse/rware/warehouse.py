...
class Task:
    ...
    workstation_position: Tuple[int, int]
    return_position: Tuple[int, int]
    phase: TaskPhase


class TaskManager:
    """Own seeded task assignment and automatic rack handling."""

    def __init__(self, warehouse):
        self.warehouse = warehouse
        self.tasks: List[Optional[Task]] = [None] * warehouse.n_agents
        self.phase_steps = np.zeros(warehouse.n_agents, dtype=np.int64)

    def reset(self):
        if len(self.warehouse.shelfs) < self.warehouse.n_agents:
            raise RuntimeError("TaskManager requires at least one rack per robot")
        self.tasks = [None] * self.warehouse.n_agents
        self.phase_steps.fill(0)
        for agent_index in range(self.warehouse.n_agents):
            self.assign(agent_index)

    def assign(self, agent_index):
        assigned_ids = {task.rack_id for task in self.tasks if task is not None}
        candidates = [
            shelf for shelf in self.warehouse.shelfs if shelf.id not in assigned_ids
        ]
        shelf = candidates[int(self.warehouse.np_random.integers(len(candidates)))]
        workstation = tuple(
            self.warehouse.goals[
                int(self.warehouse.np_random.integers(len(self.warehouse.goals)))
            ]
        )
        position = (shelf.x, shelf.y)
        self.tasks[agent_index] = Task(
            rack_id=shelf.id,
            rack_position=position,
            workstation_position=workstation,
            return_position=position,
            phase=TaskPhase.PICKUP,
        )
        self.phase_steps[agent_index] = 0
        self.sync_request_queue()
        return self.tasks[agent_index]

    def shelf(self, task):
        return self.warehouse.shelfs[task.rack_id - 1]

    def sync_request_queue(self):
        self.warehouse.request_queue = [
            self.shelf(task) for task in self.tasks if task is not None
        ]

    def target(self, task):
        if task.phase in (TaskPhase.PICKUP, TaskPhase.AUTO_PICKUP):
            return task.rack_position
        if task.phase in (TaskPhase.DELIVER, TaskPhase.AUTO_DELIVERY):
            return task.workstation_position
        return task.return_position

    def automatic_step(self, agent_index):
        agent = self.warehouse.agents[agent_index]
        task = self.tasks[agent_index]
        shelf = self.shelf(task)
        position = (agent.x, agent.y)
        if task.phase == TaskPhase.AUTO_PICKUP:
            if position != task.rack_position or agent.carrying_shelf is not None:
                raise RuntimeError("Invalid AUTO_PICKUP state")
            if (shelf.x, shelf.y) != task.rack_position:
                raise RuntimeError("Assigned rack is not at its pickup position")
            agent.carrying_shelf = shelf
            task.phase = TaskPhase.DELIVER
            duration = int(self.phase_steps[agent_index])
            self.phase_steps[agent_index] = 0
            return False, False, duration, 0, 0
        if task.phase == TaskPhase.AUTO_DELIVERY:
            if position != task.workstation_position or agent.carrying_shelf is not shelf:
                raise RuntimeError("Invalid AUTO_DELIVERY state")
            task.phase = TaskPhase.RETURN
            duration = int(self.phase_steps[agent_index])
            self.phase_steps[agent_index] = 0
            return True, False, 0, duration, 0
        if task.phase == TaskPhase.AUTO_DROP:
            if position != task.return_position or agent.carrying_shelf is not shelf:
                raise RuntimeError("Invalid AUTO_DROP state")
            agent.carrying_shelf = None
            duration = int(self.phase_steps[agent_index])
            self.tasks[agent_index] = None
            self.assign(agent_index)
            return False, True, 0, 0, duration
        return False, False, 0, 0, 0

    def begin_step(self):
        self.phase_steps += 1

    def detect_arrival(self, agent_index):
        agent = self.warehouse.agents[agent_index]
        task = self.tasks[agent_index]
        position = (agent.x, agent.y)
        if task.phase == TaskPhase.PICKUP and position == task.rack_position:
            task.phase = TaskPhase.AUTO_PICKUP
        elif task.phase == TaskPhase.DELIVER and position == task.workstation_position:
            task.phase = TaskPhase.AUTO_DELIVERY
        elif task.phase == TaskPhase.RETURN and position == task.return_position:
            task.phase = TaskPhase.AUTO_DROP
...
class Warehouse(gym.Env):
    ...
    def __init__(
        ...
        self._last_info = self._empty_metrics()

        action_count = len(Action) - 1 if self.task_manager_enabled else len(Action)
        sa_action_space = [action_count, *msg_bits * (2,)]
    ...
    def _use_slow_obs(self):
        ...
        for _ in range(self.n_agents):
            ...
            if self.task_manager_enabled:
                fields["task"] = gym.spaces.Dict(
                    OrderedDict(
                        {
                            "phase": gym.spaces.Box(0.0, 1.0, (6,), np.float32),
                            "target_delta": gym.spaces.Box(
    ...
    def _task_observation(self, agent):
        task = self.task_manager.tasks[agent.id - 1]
        phase = np.zeros(6, dtype=np.float32)
        phase[task.phase.value] = 1.0
        target_x, target_y = self.task_manager.target(task)
        delta = np.array(
            [
                (target_x - agent.x) / max(1, self.grid_size[1] - 1),
                (target_y - agent.y) / max(1, self.grid_size[0] - 1),
            ],
            dtype=np.float32,
        )
    ...
    def _get_info(self):
        return {
            ...
            for key, value in self._last_info.items()
        }
    ...
    def _empty_metrics(self):
        ...
        metrics = {
            ...
            "delivery_time": zeros.copy(),
            "return_time": zeros.copy(),
            "path_length": zeros.copy(),
            "wait_steps": zeros.copy(),
            "movement_attempts": zeros.copy(),
            "conflict_attempts": zeros.copy(),
    ...
    def step(
        ...
        for agent_index, (agent, action) in enumerate(zip(self.agents, actions)):
            if self.msg_bits > 0:
                raw_action = action[0]
            ...
            action_value = (
                ...
                else int(np.asarray(raw_action).item())
            )
            if self.task_manager_enabled and action_value == Action.TOGGLE_LOAD.value:
                raise ValueError("TOGGLE_LOAD is unavailable when TaskManager is enabled")
            if action_value < 0 or action_value >= len(Action):
                raise ValueError(f"Invalid action: {action_value}")
            task = self.task_manager.tasks[agent_index] if self.task_manager_enabled else None
            if task is not None and task.phase in (
                TaskPhase.AUTO_PICKUP,
                TaskPhase.AUTO_DELIVERY,
                TaskPhase.AUTO_DROP,
            ):
                agent.req_action = Action.NOOP
            else:
                agent.req_action = Action(action_value)
        ...
        requested_targets = [agent.req_location(self.grid_size) for agent in self.agents]
        conflict_attempts = np.zeros(self.n_agents, dtype=np.int64)
        for agent_index, (start, target) in enumerate(
            zip(before_positions, requested_targets)
        ):
            if not requested_forward[agent_index]:
                continue
            vertex_conflict = requested_targets.count(target) > 1
            edge_conflict = any(
                other != agent_index
                and before_positions[other] == target
                and requested_targets[other] == start
                for other in range(self.n_agents)
            )
            conflict_attempts[agent_index] = int(vertex_conflict or edge_conflict)
        ...
        task_delivery = False
        completed_cycles = np.zeros(self.n_agents, dtype=np.int64)
        deliveries = np.zeros(self.n_agents, dtype=np.int64)
        pickup_time = np.zeros(self.n_agents, dtype=np.int64)
        delivery_time = np.zeros(self.n_agents, dtype=np.int64)
        return_time = np.zeros(self.n_agents, dtype=np.int64)

        for agent_index, agent in enumerate(self.agents):
            agent.prev_x, agent.prev_y = agent.x, agent.y

            if self.task_manager_enabled:
                delivered, completed, pickup, delivery, returning = (
                    self.task_manager.automatic_step(agent_index)
                )
                deliveries[agent_index] = int(delivered)
                completed_cycles[agent_index] = int(completed)
                pickup_time[agent_index] = pickup
                delivery_time[agent_index] = delivery
                return_time[agent_index] = returning
                if delivered:
                    task_delivery = True
                    if self.reward_type == RewardType.GLOBAL:
                        ...
                        rewards[agent_index] += 0.5
                if agent.req_action == Action.NOOP and self.task_manager.tasks[
                    agent_index
                ].phase in (TaskPhase.DELIVER, TaskPhase.RETURN, TaskPhase.PICKUP):
                    continue
        ...
        shelf_delivered = task_delivery
        for y, x in (() if self.task_manager_enabled else self.goals):
        ...
        if self.task_manager_enabled:
            for agent_index in range(self.n_agents):
                self.task_manager.detect_arrival(agent_index)

        after_positions = [(agent.x, agent.y) for agent in self.agents]
        path_length = np.array(
            [before != after for before, after in zip(before_positions, after_positions)],
            dtype=np.int64,
        )
        movement_denied = (requested_forward & (path_length == 0)).astype(np.int64)
        wait_steps = (path_length == 0).astype(np.int64)
        ...
        self._last_info = {
            ...
            "movement_attempts": requested_forward.astype(np.int64),
